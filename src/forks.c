/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   forks.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joe <joe@student.42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/07 12:47:14 by joe               #+#    #+#             */
/*   Updated: 2023/10/07 15:13:17 by joe              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/philosopher.h"

void	drop_left_fork(t_data_each_philos *philo)
{
	pthread_mutex_unlock(philo->left_f);
}

void	drop_right_fork(t_data_each_philos *philo)
{
	pthread_mutex_unlock(philo->right_f);
}

int	take_left_fork(t_data_each_philos *philo)
{
	if (philo_died(philo) || get_philo_state(philo) == DEAD)
		return (1);
	pthread_mutex_lock(philo->left_f);
	print_msg(philo->info_philos, philo->id, TAKE_FORKS_LEFT);
	return (0);
}

int	take_right_fork(t_data_each_philos *philo)
{
	if (philo_died(philo) || get_philo_state(philo) == DEAD)
		return (1);
	pthread_mutex_lock(philo->right_f);
	print_msg(philo->info_philos, philo->id, TAKE_FORKS_RIGHT);
	return (0);
}

int	take_forks(t_data_each_philos *philo)
{
	if (get_nb_philos(philo->info_philos) == 1)
		return (just_one_philo(philo));
	if (philo->id % 2 == 0)
	{
		if (take_right_fork(philo) != 0)
			return (1);
		if (take_left_fork(philo) != 0)
		{
			drop_right_fork(philo);
			return (1);
		}
	}
	else
	{
		if (take_left_fork(philo) != 0)
			return (1);
		if (take_right_fork(philo) != 0)
		{
			drop_left_fork(philo);
			return (1);
		}
	}
	return (0);
}
